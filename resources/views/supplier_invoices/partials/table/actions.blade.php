...
<div class="btn-group">
    @if ($entity->pdf)
        <a 
            href="{{ route('supplier-invoices.view', $entity->id) }}" 
            target="_blank" 
            class="btn btn-info" 
            title="Prikaži PDF"
        >
    ...
    @endif
    <a 
        href="{{ route('supplier-invoices.edit', ['entity' => $entity->id]) }}" 
...
<form id="uploadPdfForm" action="{{ route('supplier-invoices.upload_pdf') }}" method="POST" enctype="multipart/form-data" style="display: none;">
    @csrf
